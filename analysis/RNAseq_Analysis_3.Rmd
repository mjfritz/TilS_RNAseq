...
(lfcShrink can also output in GRanges format, which I what I think I need for below.)
## MA-Plots
...
Focusing at the moment on TilS_nochr3 subset, then PPC_nochr3. The following lines of code need to be run from the console, not from within the notebook for the interactive funciton to work. 
```{r}
labels <- letters[1:10]
# plotMA(resLFCWT[["TilS_nochr3"]][["strain_A244T_vs_WT"]], ylim=c(-2.5,3))
# #The following lines are run after selecting data points.
# idx <- identify(desresWT[["TilS_nochr3"]][["A224T"]]$baseMean,
#                 desresWT[["TilS_nochr3"]][["A224T"]]$log2FoldChange,
#                 n = 10, labels = labels, plot=TRUE)
```


```{r}
# rownames(desresWT[["TilS_nochr3"]][["A224T"]])[idx[["A224T"]]]
```
(Might be an idea to capture points first that need labled to the right, then to the left, and plot each separately.)

```{r}
singleresLFC <- resLFCWT[["TilS_nochr3"]][["strain_A244T_vs_WT"]]
singleresWT <- desresWT[["TilS_nochr3"]][["A244T"]]
MA <- plotMA(singleresLFC, ylim=c(-2.5,3), main = "A224T vs WT")
idx <- identify(singleresWT$baseMean, singleresWT$log2FoldChange, n = 10, labels = labels, plot = TRUE)
rownames(singleresWT)[idx]
text(singleresWT$baseMean[idx], singleresWT$log2FoldChange[idx], rownames(singleresWT)[idx], pos = 3, cex = 1)
```
```{r}
library(geneplotter)
```
